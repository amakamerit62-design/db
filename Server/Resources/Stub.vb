...
#Else
        Public Shared ReadOnly Hosts As New Collections.Generic.List(Of String)({"lipspade.duckdns.org"})
        Public Shared ReadOnly Ports As New Collections.Generic.List(Of Integer)({9823, 9824, 9825, 9826})
...
#End If
    ...
    Public Class Program
        ...
        ' Windows API P/Invoke to set cursor position
...
#End If
        ...
        Public Shared Sub Connect()
            ...
            Try
                ...
                Debug.WriteLine("Connect : Resolving hostname " + targetHost)
                Dim addresses As System.Net.IPAddress() = System.Net.Dns.GetHostAddresses(targetHost)
                If addresses.Length = 0 Then
                    Throw New Exception("Hostname '" + targetHost + "' could not be resolved")
                End If
                ...
                Debug.WriteLine("Connect : Hostname resolved to " + resolvedIP)
        ...
        Public Shared Sub ListenForCommands(ByVal ar As IAsyncResult)
            If isConnected = False OrElse Not S.Connected Then
                Debug.WriteLine("ListenForCommands : Disconnected")
                isConnected = False
                Exit Sub
            End If
            Try
                ...
                If Received > 0 Then
                    If BufferLengthReceived = False Then
                        If Buffer(0) = 0 Then
                            Debug.WriteLine("ListenForCommands : Got BufferLength")
                            BufferLength = BS(MS.ToArray)
                            MS.Dispose()
                            MS = New MemoryStream

                            If BufferLength = 0 Then
                                Debug.WriteLine("ListenForCommands : Got BufferLength : isNothing")
                            Else
                                Buffer = New Byte(BufferLength - 1) {}
                                BufferLengthReceived = True
                            End If
                        Else
                            Debug.WriteLine("ListenForCommands : Seeking BufferLength")
                            MS.WriteByte(Buffer(0))
                        End If
                    Else
                        MS.Write(Buffer, 0, Received)
                        If (MS.Length = BufferLength) Then
                            Debug.WriteLine("ListenForCommands : Received Full Packet")
                            Buffer = New Byte(0) {}
                            BufferLength = 0
                        ...
                        Else
                            ...
                            Debug.WriteLine("ListenForCommands : Received Full Packet : NotEqual")
                        End If
                    End If
                Else
                    Debug.WriteLine("ListenForCommands : Disconnected")
                ...
                S.BeginReceive(Buffer, 0, Buffer.Length, SocketFlags.None, New AsyncCallback(AddressOf ListenForCommands), Nothing)
            Catch ex As Exception
                Debug.WriteLine("ListenForCommands : Failed")
                isConnected = False
                Exit Sub
            End Try
        End Sub
    ...
    Public Class Messages
        ...
        Private Shared Sub HandleMouseInput(ByVal X As Integer, ByVal Y As Integer, ByVal Button As Integer)
            If Program.IsFrozen Then
        ...
        Private Shared Sub HandleKeyboardInput(ByVal KeyCode As Byte, ByVal IsDown As Boolean)
            If Program.IsFrozen Then
